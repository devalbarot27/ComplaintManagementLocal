...
$allowedOrderColumns = ['id', 'role_name', 'description', 'created_at'];

$req = dt_parse_request($allowedOrderColumns, 'id');
$baseWhere = 'deleted_at IS NULL';
$filterParams = [];

$recordsTotalStmt = $obconn->prepare("SELECT COUNT(*) AS total FROM roles WHERE {$baseWhere}");
...
if ($req['searchValue'] !== '') {
    $searchFilter = role_search_filter($req['searchValue']);
    $filterWhere .= ' AND ' . $searchFilter['sql'];
    $filterParams = array_merge($filterParams, $searchFilter['params']);
}

$countFilteredStmt = $obconn->prepare("SELECT COUNT(*) AS total FROM roles WHERE {$filterWhere}");
...
foreach ($dataStmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $data[] = [
        ...
        'actions' => role_entry_actions((int) $row['id']),
    ];
}
